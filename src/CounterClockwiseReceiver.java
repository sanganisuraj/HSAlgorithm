...
public class CounterClockwiseReceiver implements Runnable {

    private final BlockingQueue<Message> q;
    private final BlockingQueue<Long> pq;

    public CounterClockwiseReceiver(BlockingQueue<Message> q,
            BlockingQueue<Long> pq) {
        this.q = q;
        this.pq = pq;
    }
    ...
    public void run() {
        synchronized (q) {
            ...
            while (i.hasNext()) {
                m = i.next();
                if (m.getDirection() == Direction.Counterclockwise && m.getFlag().equals(Flag.out)) {
                    q.remove(m);
                    break;
                } else {
                    m = new Message();
            ...
            while (hops > 0) {
                if (m.getMessageReceiver() < m.getMessageSender() && hops > 1 && m.getFlag() == Flag.out) {
                    q.remove(m);
                    int messageReceiverIndex = Master.ids.indexOf(m.getMessageReceiver()) - 1 < 0 ? Master.slaveCount - 1 : Master.ids.indexOf(m.getMessageReceiver()) - 1;
